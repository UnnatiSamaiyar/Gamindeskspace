...
router.put("/employee/:id", verifyTokenAndRoles(["admin"]),async (req, res) => {
  try {
    ...
    if(employee.role !=='admin'  &&  ( req.body.productCategory !== employee.productCategory) ) {
          return res.status(400).json({message:"Emp category can not be changed for non-admin role!"});
    }
    ...
    employee.productCategory = req.body.role==='admin' ? null : req.body.productCategory; 
...
router.get("/employee", verifyTokenAndRoles(["admin", "Account Manager", "NOC Manager"]),async (req, res) => {
  try {
    ...
    if (req.employee.role === 'admin') {
      // Fetch all employees from the database
      employees = await Employee.find().populate('createdBy updatedBy').sort({ createdAt: 'desc' });
    } else if (req.employee.role === 'Account Manager') {
      // Account Managers can access users with the roles 'Account Manager' or 'NOC Manager'
      employees = await Employee.find({ role: { $in: ['Account Manager', 'NOC Manager'] } }).sort({ createdAt: 'desc' });
    } else {
      // NOC can only access employees with the role 'NOC Manager'
      employees = await Employee.find({ role: 'NOC Manager' }).sort({ createdAt: 'desc' });
    }
...
router.get("/employeeActiveAM_VOICE", verifyToken,async (req, res) => {
  try {
    let employees;
    employees = await Employee.find({ role: { $in: ['Account Manager'] },status:true,productCategory:"VOICE"}).sort({ cretedAt: 'desc' });
    res.status(200).json(employees);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
...
router.get("/employeeActiveAM_SMS", verifyToken,async (req, res) => {
  try {
    let employees;
    employees = await Employee.find({ role: { $in: ['Account Manager'] },status:true,productCategory:"SMS" }).sort({ cretedAt: 'desc' });
    res.status(200).json(employees);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
...
router.get("/employeeActiveNOC_VOICE", verifyToken,async (req, res) => {
  try {
    let employees;
    employees = await Employee.find({ role: { $in: ['NOC Manager'] },status:true,productCategory:"VOICE" }).sort({ cretedAt: 'desc' });
    res.status(200).json(employees);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
...
router.get("/employeeActiveNOC_SMS", verifyToken,async (req, res) => {
  try {
    let employees;
    employees = await Employee.find({ role: { $in: ['NOC Manager'] },status:true,productCategory:"SMS" }).sort({ cretedAt: 'desc' });
    res.status(200).json(employees);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal server error" });
...
router.post('/login', async (req, res) => {
  try {
    ...
    const hashedInputPassword = CryptoJS.SHA256(req.body.password).toString(CryptoJS.enc.Hex);
...
router.post("/resetPasswordRequest",async (req,res)=>{
  ...
  const token = jwt.sign({ emp_id: user._id },  process.env.JWT_SEC, { expiresIn: '1h' });
...
router.post("/resetPassword",async (req,res)=>{
  ...
  try{
      const { userId, token, newPassword } = req.body;
       console.log(req.body);
      const resetToken = await Employee.findOne({ _id:userId, passwordResetToken:token, passwordResetTokenExpiry: { $gt: Date.now() } });
      if (!resetToken) {
...
router.post("/changePassword",verifyToken,async (req,res)=>{
  ...
  try{
    ...
    const hashedPasswordNew = CryptoJS.SHA256(newPassword).toString(CryptoJS.enc.Hex);
...
router.post("/changePermissions/:id", verifyTokenAndRoles(["admin", "Account Manager"]), async (req, res) => {
  try {
