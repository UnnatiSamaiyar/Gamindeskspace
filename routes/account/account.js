...
router.post("/account", verifyTokenAndRoles(['admin', 'Account Manager']), async (req, res) => {
  try {
    const { name, email, phone, address,country,companyName,ghostName,skypeId, timeZone, billingEmail,salesEmail,paymentCycle,currency, createdBy ,assignedTo,accountType,remarks} = req.body;
    ...
    const newAccount = new Account({
        name, email, phone, address,country,currency,companyName,ghostName, timeZone, skypeId, billingEmail,salesEmail,paymentCycle,
        createdBy ,assignedTo,accountType,remarks
    });
    ...
    const notification=new Notification(
      {
        message:"New Account : "+savedAccount.ghostName+" created by "+emp.name,
     ...
     if(accountType==='VOICE')
      req.io.emit('AM_VOICE',notification);
    else  if(accountType==='SMS')
...
router.get("/account", verifyToken,async (req, res) => {
    try {
        ...
        const query = requester.role === 'admin' && (!requester?.productCategory || requester?.productCategory === null)
          ? {}
      ...
      // Process each account to append services and servicesCount
        const accountsWithServices = await Promise.all(accounts.map(async (account) => {
...
router.get("/account/:id", verifyToken, async (req, res) => {
    ...
    try {
        const account = await Account.findById(req.params.id).populate(populateFields)
...
router.put("/account/changeAssignedTo", verifyToken, async (req, res) => {
    ...
    try {
      const {accountId,assignedTo,empId} = req.body;
        ...
        if(!empBy)
        return  res.status(404).json({message:"empBy not Exist"});  
...
router.put("/account/:id", verifyTokenAndRoles(['admin', 'Account Manager']), async (req, res) => {
    try {
      const { name, email, phone, address,country,companyName,ghostName,skypeId, timeZone, billingEmail,salesEmail,paymentCycle,currency,remarks} = req.body;
...
router.delete("/account/flush/:id",verifyTokenAndAdmin,async (req,res)=>{
  ...
  const taskObjList =  await Task.find({ service: { $in: servicesObjList.map(service => service._id) } });



  const resultThread =  await Thread.deleteMany({ task: { $in: taskObjList.map(task => task._id) } });

  const resultTask = await Task.deleteMany({ service: { $in: servicesObjList.map(service => service._id) } });

  const resultServices = await Services.deleteMany({ accountId: obj._id });

  const resultAccount =  await Account.deleteOne({_id:obj._id});

    return res.status(200).json({message:'Deleted',resultThread,resultTask,resultServices,resultAccount});
  }catch(error){
    return res.status(500).json({message:error.message})
  }
});
